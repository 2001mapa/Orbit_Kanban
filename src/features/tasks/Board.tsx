                              ...
                              >
                                <div className="flex flex-col gap-2.5">
                                  ...
                                  {task.description && (
                                    ...
                                    {/* BADGES SECTION */}
                                    {(task.priority === 'high' || task.priority === 'urgent' || (task.tags && task.tags.length > 0) || task.assigned_to) ? (
                                      <div className="flex flex-wrap gap-1.5 mt-2 border-t border-stone-100 pt-3">
                                        {task.priority && (task.priority === 'high' || task.priority === 'urgent') && (
                                          <span className="text-[10px] md:text-[11px] font-bold px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-200 flex items-center gap-1">
                                            ⚡ {task.priority === 'urgent' ? 'Urgente' : 'Alta Prioridad'}
                                          </span>
                                        )}
                                        
                                        {task.tags && task.tags.length > 0 && task.tags.map((tag, i) => (
                                          <span key={i} className="text-[10px] md:text-[11px] font-bold px-2 py-0.5 rounded-full bg-teal-50 text-teal-700 border border-teal-200 flex items-center gap-1">
                                            ✓ {tag}
                                          </span>
                                        ))}
                                        ...
                                        {task.assigned_to && (
                                          <span className="text-[10px] md:text-[11px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 flex items-center gap-1">
                                            👨‍💻 Asignada a {getAssigneeLabel(task.assigned_to)}
                                          </span>
                                        )}
                                      </div>
                                    ) : null}
